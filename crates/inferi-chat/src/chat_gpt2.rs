...
use inferi::models::gpt2::cpu::Gpt2Params;
use inferi::models::gpt2::{Gpt2, Gpt2State, Gpt2Weights};
use inferi::ops::BatchedMultiqueryAttentionParams;
...
impl ChatGpt2 {
    pub fn from_gguf(backend: &Device, gguf: &Gguf) -> anyhow::Result<ChatGpt2> {
        let transformer = Gpt2::new(backend)?;
        let config = Gpt2Params::from_gguf(gguf);
        let weights = Gpt2Weights::from_gguf(backend, &config, gguf);
        let tokenizer = AnyTokenizer::from_gguf(gguf)?;
        let state = Gpt2State::new(backend, &config);
        ...
        Ok(Self {
            ...
            config,
            state,
            shapes: TensorLayoutBuffers::new(),
        })
    }
