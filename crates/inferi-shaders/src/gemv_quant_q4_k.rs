...
/// Dequantize Q4_K block for a workgroup thread.
#[inline]
fn dequantize_q4_k_workgroup(m: &[u32], block_id: u32, k: u32) -> [Vec4; 2] {
    let d_dmin_id = (block_id * BLOCK_Q4K_SIZE) as usize;
...
pub fn gemv(
    ...
    for j in 0..shape_m.w {
        ...
        let parts0 = dequantize_q4_k_workgroup(m, quant0, lid);
        let parts1 = dequantize_q4_k_workgroup(m, quant1, lid);
        let parts2 = dequantize_q4_k_workgroup(m, quant2, lid);
        let parts3 = dequantize_q4_k_workgroup(m, quant3, lid);
