...
// use khal::re_exports::bytemuck;
// use khal::re_exports::Device;
...
impl ChatLlama2 {
    ...
    pub async fn forward(
        ...
        // FIXME: this will result in re-calculating the whole context instead of just re-using the
        //        existing KV-cache from past prompts/responses. To fix this, we need to account for
        //        the fact that, after the assistant is done responding, we would need to remove
        //        the KV-cache lines matching tokens obtained during reasoning. This is necessary
        //        because reasoning will be stripped from the next prompt (so it doesn't count in
        //        the context; which is good since reasoning is very verbose).
        let start_pos = 0;
        ...
        for pos in start_pos.. {
            ...
            self.forward_logits(backend, pos as u32, token as u32, &mut logits)
                .await?;
            let _elapsed = _t0.elapsed().as_secs_f64();
            ...
            if pos + 1 >= prompt_toks.len() {
                if token == self.tokenizer.eos() {
                    // println!("Exitting at current pos: {}", pos);
                    out(ChatEvent::Finish { next_pos: pos + 1 })?;
                    break;
                } else {
    ...
    async fn forward_logits(
        ...
        // println!("init time: {}", t0.elapsed().as_secs_f64());
        ...
        // if pos == 1 {
        // let debug = backend
        //     .slow_read_vec(state.q.buffer())
        //     .await?;
        // println!("debug: {:?}", &debug[..10]);
        // std::process::exit(0);
        // }
