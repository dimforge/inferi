...
pub enum GgufParseError {
    #[error(
        "the input file isn’t a ggml binary file. Got a magic number of {0:#x} instead of 0x46554747 or 0x47475546."
    )]
...
impl GgufTensorData {
    ...
    pub fn to_gpu_quant(
        ...
        dims: [u32; 4],
        rank: u32,
    ) -> Result<Option<GpuQuantTensor>, GpuBackendError> {
        self.to_gpu_quant_print(backend, dims, rank, false)
    }

    pub fn to_gpu_quant_print(
        &self,
        backend: &GpuBackend,
        dims: [u32; 4],
        rank: u32,
        ...
        macro_rules! match_quant {
            ($($match_id: ident, $gpu_ty: ident);*) => {
                match self {
                    $(Self::$match_id(v) => {
                        if v.len() % 2 == 0 && dims[1] as usize % $gpu_ty::DEQUANTIZED_LEN == 0 {
                            // This can be interpreted as a BlockQ8_0x2 matrix.
                            let gpu_data: &[$gpu_ty] = bytemuck::cast_slice(v);
                            let mut dims = dims;
                            dims[1] /= $gpu_ty::DEQUANTIZED_LEN as u32;
                            let tensor = TensorBuilder::tensor(&dims[..rank as usize], BufferUsages::STORAGE)
                                .build_init(backend, gpu_data)?;
                            return Ok(Some(GpuQuantTensor::$match_id(tensor)));
                        } else {
        ...
        self.dequantize()
            .map(|deq| {
                // This can be interpreted as a BlockQ8_0x2 matrix.
                let tensor = TensorBuilder::tensor(&dims[..rank as usize], BufferUsages::STORAGE)
                    .build_init(backend, &deq)?;
                Ok(GpuQuantTensor::F32(tensor))
            })
            .transpose()
    }
}
...
macro_rules! impl_from_metadata(
    ($($Variant: ident ( $t: ty )),*) => {$(
        impl From<$t> for GgufMetadataValue {
            fn from(v: $t) -> Self {
                Self::$Variant(v)
            }
        }
...
pub struct GgufTensor {
    /// The tensor dimensions, given as [nrows, ncols, nmats, ncubes].
    ///
    /// Note that, compared to the actuall GGUF format, the first two elements are swapped
    /// (GGUF originally stores `ncols` first and `nrows` second).
    pub dimensions: [u64; 4], // Currently at most 4, but this may change in the future
    pub rank: u32,
...
impl GgufTensor {
    ...
    /// Loads the tensor in the GPU, assuming row-major ordering.
    pub fn to_gpu_quant(
        &self,
        backend: &GpuBackend,
    ) -> Result<Option<GpuQuantTensor>, GpuBackendError> {
        let dims = self.dimensions.map(|dim| dim as u32);
        self.data.to_gpu_quant(backend, dims, self.rank)
    }
    ...
    pub fn to_gpu_quant_print(
        ...
        let dims = self.dimensions.map(|dim| dim as u32);
        self.data
            .to_gpu_quant_print(backend, dims, self.rank, print)
    }
}
...
impl Gguf {
    ...
    fn load_tensors(
        ...
        for _ in 0..header.tensor_count {
            ...
            let rank: u32 = read_pod(bytes, offset)?;
            let mut dimensions = [1u64; 4];

            assert!(
                rank <= 4,
                "Tensors of rank larger than 4 are not supported yet."
            );
            ...
            // NOTE: GGUF stores tensor dimensions as WHCN whereas we store them in NCHW format.
            //       So we need to flip all the dimensions.
            dimensions[..rank as usize].reverse();
...
pub(crate) fn read_array_unaligned<T: Pod>(
    len: usize,
    bytes: &[u8],
    offset: &mut usize,
