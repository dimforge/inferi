...
pub fn sam_encode_image(
    ...
    ctxt.add_assign(&mut cur, &to_add)?;
    ...
    ctxt.add_assign(&mut cur, &enc.pe)?;
    ...
    for il in 0..n_enc_layer {
        ...
        // ref: https://github.com/facebookresearch/segment-anything/blob/main/segment_anything/modeling/image_encoder.py#L168
        {
            ...
            ctxt.mul_assign(&mut cur, layer.norm1_w.as_view().canonicalize())?;
            ctxt.add_assign(&mut cur, layer.norm1_b.as_view().canonicalize())?;
        }
        ...
        // self-attention
        {
            ...
            ctxt.add_assign(&mut cur, layer.qkv_b.as_view().canonicalize())?;
            ...
            ctxt.scale_assign(&mut kq, 1.0 / (n_enc_head_dim as f32).sqrt())?;
            ...
            ctxt.add_rel_pos_assign(&mut kq, &rel_w, &rel_h)?;

            let mut attn = kq;
            ctxt.softmax_rows(&mut attn)?;
            ...
            ctxt.add_assign(&mut cur, layer.proj_b.as_view().canonicalize())?;
        }
        ...
        ctxt.mul_assign(&mut cur, layer.norm2_w.as_view().canonicalize())?;
        ctxt.add_assign(&mut cur, layer.norm2_b.as_view().canonicalize())?;

        // fully connected
        cur = ctxt.matmul_ggml(&layer.mlp_lin1_w, &cur)?;
        ctxt.add_assign(&mut cur, layer.mlp_lin1_b.as_view().canonicalize())?;
        ...
        ctxt.add_assign(&mut cur, layer.mlp_lin2_b.as_view().canonicalize())?;
