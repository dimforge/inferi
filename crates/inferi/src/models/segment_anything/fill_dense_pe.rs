...
pub fn sam_fill_dense_pe(
    ...
    let pe_transposed = ctxt.contiguous(enc.pe.as_view().transpose_last_dims())?;
    ...
    // ref: https://github.com/facebookresearch/segment-anything/blob/main/segment_anything/modeling/prompt_encoder.py#L192
    {
        ...
        let stride = [Some(1), Some(cur.stride_ggml(1)), Some(cur.stride_ggml(2))];
        ctxt.copy(
        ...
        let shape = [t_sin.size_ggml(0), t_sin.size_ggml(1), t_sin.size_ggml(2)];
        let stride = [Some(1), Some(cur.stride_ggml(1)), Some(cur.stride_ggml(2))];
        ctxt.copy(
