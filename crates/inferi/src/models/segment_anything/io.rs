...
pub struct SamGgmlFile {
    pub params: HParams,
    pub weights: HashMap<String, ([u32; 4], u32, Vec<f32>)>,
}
...
impl SamGgmlFile {
    pub fn into_gguf(self) -> Gguf {
        ...
        for (name, (mut ne, mut rank, data)) in self.weights {
            // Account for ggml’s swaped row/cols indices.
            ne.swap(0, 1);
            if rank < 2 && ne[1] != 1 {
                // Adjust the rank if the swapping effectively makes it a matrix with a single row.
                rank = 2;
            }
    ...
    fn load_weights(
        bytes: &[u8],
        offset: &mut usize,
    ) -> Result<HashMap<String, ([u32; 4], u32, Vec<f32>)>, SamGgmlParseError> {
        let mut weights = HashMap::new();

        while *offset < bytes.len() {
            // Number of tensor dimensions.
            let rank: u32 = Self::read_pod(bytes, offset)?;
            ...
            println!(
                "Found tensor with name: {}, elements: {}, dimensions: {:?}, rank: {:?}",
                name, nelements, ne, rank
            );
            ...
            weights.insert(name.into_owned(), (ne, rank, data));
        }
