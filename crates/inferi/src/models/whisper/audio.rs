...
fn fft<T: RealField + Copy>(inp: &[T]) -> Vec<T> {
    let n = inp.len();
    let zero = T::zero();
    if n == 1 {
...
fn dft<T: RealField + Copy>(inp: &[T]) -> Vec<T> {
    let zero = T::zero();
    let n = inp.len();
    let two_pi = T::two_pi();
...
fn log_mel_spectrogram_w<T: RealField + Copy>(
    ith: usize,
    hann: &[T],
    samples: &[T],
    filters: &[T],
    ...
    for i in (ith..end).step_by(n_threads) {
        ...
        for j in 0..n_mel {
            ...
            mel[j * n_len + i] = T::max(sum, T::from_f64(1e-10).unwrap()).log10();
        }
    }
    mel
}

pub fn log_mel_spectrogram_<T: RealField + Copy>(
    samples: &[T],
    filters: &[T],
    fft_size: usize,
    fft_step: usize,
    ...
    let fft_size_t = T::from_usize(fft_size).unwrap();

    let hann: Vec<T> = (0..fft_size)
        .map(|i| half * (one - ((two_pi * T::from_usize(i).unwrap()) / fft_size_t).cos()))
        .collect();
    ...
    let n_threads = 12; // TODO: make number of threads configurable. std::cmp::min(get_num_threads() - get_num_threads() % 2, 12);
...
pub fn pcm_to_mel<T: RealField + Copy>(
    cfg: &super::WhisperConfig,
    samples: &[T],
    filters: &[T],
) -> Vec<T> {
